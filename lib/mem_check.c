...
// A descriptor store information about memory blocks
...
#define hash(ptr, table) (((unsigned long)(ptr) >> 3) & (sizeof(table)/sizeof((table)[0])-1))

#define NALLOC ((4096 + sizeof(union align) - 1) / (sizeof(union align))) * (sizeof(union align))

static struct descriptor freelist = { &freelist, 0, 0, 0, 0, 0 }; /* make a circle linked list */

/*
 * given an address find the target descriptor. return NULL or
 * a vaild descriptor structure.
 */
...
static struct descriptor *dalloc(void *ptr, long size, const char *file, int line) 
{
    ...
    if (nleft <= 0) {
        ...
        avail = (struct descriptor *)malloc(NDESCRIPTOR * sizeof(*avail));
        if (avail == NULL)
            return NULL;
        nleft = NDESCRIPTOR;
    }
    ...
    for (bp = freelist.free; bp; bp = bp->free) {
        if (bp->size > nbytes) {
            ...
            if ((bp = dalloc(ptr, nbytes, file, line)) != NULL) {
                ...
                err_exit(errno, "dalloc error", file, line);
            }
        }
        /* come to end of linked list */
        if (bp == &freelist) {
...
/*
 * case 1: valid pointers
 * case 2: invalid pointers
 * case 3: multi-free 
 */
void Mem_free(void *ptr, const char *file, int line) {
    ...
    if (ptr) {
        struct descriptor *bp;
        // check whether ptr is a valid pointer.
...
void *Mem_resize(void *ptr, long nbytes, const char *file, int line) {
    ...
    if (((unsigned long)ptr)%(sizeof(union align)) != 0 || (bp = find(ptr)) == NULL || bp->free)
        //Except_raise(&Assert_Failed, file, line);
        err_sys("invalid pointer", file, line);
